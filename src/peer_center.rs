...
use crate::proto::peer_rpc::{
    DirectConnectedPeerInfo, GetGlobalPeerMapRequest, GetGlobalPeerMapResponse, PeerInfoForGlobalMap,
...
/// PeerCenter state manager.
/// Mirrors the logic in easytier/src/peer_center/server.rs.
#[derive(Default)]
pub struct PeerCenter {
    groups: HashMap<String, PeerCenterGroupData>,
}

impl PeerCenter {
    pub fn new() -> Self {
        PeerCenter { groups: HashMap::new() }
    }
    ...
    /// `report_bytes` is encoded `ReportPeersRequest`.
    /// Returns encoded `ReportPeersResponse` bytes.
    pub fn report_peers(&mut self, group_key: &str, report_bytes: &[u8]) -> Result<Vec<u8>, String> {
        let req =
            ReportPeersRequest::decode(report_bytes).map_err(|e| format!("decode ReportPeersRequest failed: {}", e))?;
        ...
        let mut buf = Vec::new();
        prost::Message::encode(&resp, &mut buf).map_err(|e| format!("encode ReportPeersResponse failed: {}", e))?;
        Ok(buf)
    }
    ...
    pub fn get_global_peer_map(&mut self, group_key: &str, request_bytes: &[u8]) -> Result<Vec<u8>, String> {
        let req = GetGlobalPeerMapRequest::decode(request_bytes)
            .map_err(|e| format!("decode GetGlobalPeerMapRequest failed: {}", e))?;
        ...
        if digest == data.digest && digest != 0 {
            ...
            prost::Message::encode(&resp, &mut buf)
                .map_err(|e| format!("encode GetGlobalPeerMapResponse failed: {}", e))?;
            return Ok(buf);
        }
    ...
    pub fn clean_outdated(&mut self, ttl_sec: u64) {
        ...
        for data in self.groups.values_mut() {
            data.peer_report_time.retain(|_, v| now_ms - *v < ttl_ms);
            data.global_peer_map.retain(|_, v| now_ms - v.update_time_ms < ttl_ms);
        }
        self.groups
            .retain(|_, data| !data.global_peer_map.is_empty() || !data.peer_report_time.is_empty());
    }

    /// Return the current digest for a group.
    pub fn calc_digest(&self, group_key: &str) -> u64 {
        self.groups.get(group_key).map(|d| d.digest).unwrap_or(0)
    }
}
